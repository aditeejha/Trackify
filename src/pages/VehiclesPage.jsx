...
import { mockVehicles, vehicleStatuses } from '../data/fleetData'

const VehicleRow = ({ vehicle, onViewClick }) => {
  ...
  return (
    <tr className="border-b hover:bg-gray-50 cursor-pointer" onClick={onViewClick}>
      ...
      <td className="px-6 py-4 text-sm text-gray-600">{vehicle.assignedDriver ? 'Assigned' : 'Unassigned'}</td>
    </tr>
  )
}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredVehicles.map((vehicle) => (
              <VehicleRow key={vehicle.id} vehicle={vehicle} onViewClick={() => navigate(`/vehicles/${vehicle.id}`)} />
            ))}
          </tbody>
        </table>
      </div>
